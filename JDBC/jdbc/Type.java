...
public class Type{
  ...
  public Type(Connection con, String categorie, boolean estUnique, boolean estDescendante, boolean estRevocable, boolean estLimitee){
    Statement salle = con.createStatement("SELECT id_salle FROM SALLE as S, VENTE as V WHERE S.categorie =?, S.id_salle=V.id_salle, V.unicite_enchere=?, V.sens_vente=?, V.annulation_vente=?, V.duree_vente=?");
    ...
    if(rs.next()){
      this.idSalle = rs.getIn("id_salle");
    }
  }
